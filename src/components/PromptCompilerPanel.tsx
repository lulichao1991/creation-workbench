...
export function PromptCompilerPanel({ projectPath, projectId, taskId, revision, officialPrompt, onRefresh, onError }: Props) {
  ...
  return <section className="prompt-compiler-panel">
    <div className="prompt-compiler-heading"><div><strong>提示词编译器</strong><small>仅编译与留痕，不调用视频模型。</small></div><button className="primary" disabled={busy || !profileKey || !templateId} onClick={() => void compile()}>{busy ? "处理中…" : "编译新版本"}</button></div>
    ...
    {selected && <>
      ...
      {selected.warnings.length > 0 && <div className="prompt-warning-list">{selected.warnings.map((warning, index) => <div className={`prompt-warning ${warning.severity}`} key={`${warning.code}-${index}`}><strong>{warning.code}</strong><span>{warning.message}</span></div>)}</div>}
      <details className="prompt-source-map" open={selected.referenceImages.length > 0}><summary>需上传的参考图（{selected.referenceImages.length}）</summary>{selected.referenceImages.length ? selected.referenceImages.map((reference) => <div key={`${reference.sourceType}:${reference.sourceId}`}><strong>{reference.label}</strong><span>{reference.sourceType} · {reference.filePath}</span></div>) : <small>本次编译没有正式参考图</small>}</details>
      ...
      <div className="prompt-compiler-actions"><button className="ghost" onClick={() => setCompare((value) => !value)}>{compare ? "收起对比" : "对比当前正式稿"}</button><button className={confirming ? "primary" : "secondary"} disabled={busy || !draft.trim()} onClick={() => void setCurrent()}>{confirming ? "再次点击确认设为正式稿" : selected.status === "current" && draft === officialPrompt ? "已是当前正式稿" : "设为当前正式稿"}</button>{confirming && <button className="ghost" onClick={() => setConfirming(false)}>取消</button>}</div>
      ...
      <details className="prompt-source-map"><summary>查看来源映射（{selected.sourceMap.length}）</summary>{selected.sourceMap.map((entry, index) => <div key={`${entry.sourceId}-${index}`}><strong>{entry.label}</strong><span>{entry.sourceType} · {entry.sourceId}</span></div>)}</details>
    </>}
  </section>;
}
